...
def counted(spend: Mapping[str, Usage]) -> tuple[float, list[str]]:
    """`(what is known to have been spent, the keys whose price is unknown)`."""
    total = sum(u.cost_usd for u in spend.values() if u.cost_usd is not None)
    return float(total), sorted(k for k, u in spend.items() if u.cost_usd is None)


def refusal(spend: Mapping[str, Usage], ceiling: float | None, subject: str) -> str | None:
    ...
    unknown = (
        f", and {len(unpriced)} more reported no price ({', '.join(unpriced)})"
        if unpriced
        else ""
    )
    return (
