...
1. Write a starter config at `~/.orchestrator-mcp/config.yaml` from the agent CLIs installed here:
   ...
   ```
   uvx orchestrator-mcp-server@0.7.3 init --host claude
   ```
...
2. Check it with the same environment the plugin gives the server:
   ...
   ```
   ORCHESTRATOR_CONFIG="$HOME/.orchestrator-mcp/config.yaml" ORCHESTRATOR_HOST_RUNTIME=claude uvx orchestrator-mcp-server@0.7.3 doctor
   ```
